...
public class SysUserController extends BaseController
{
    ...
    public AjaxResult menuTreeselect(@PathVariable("userId") Long userId)
    {
        Long currentUserId = SecurityUtils.getUserId();
        List<SysMenu> menus = menuService.selectMenuList(currentUserId);
        AjaxResult ajax = AjaxResult.success();
        ...
        {
            ajax.put("checkedKeys", menuService.selectMenuListByRoleId(personalRole.getRoleId()));
        }
        return ajax;
    }
     ...
     */
    @RequiresPermissions("system:user:edit")
    @Log(title = "用户菜单权限", businessType = BusinessType.GRANT)
    @PutMapping("/authMenu")
    public AjaxResult authMenu(@RequestBody SysUser user)
    {
        userService.checkUserAllowed(new SysUser(user.getUserId()));
        userService.updateUserMenuAuth(user.getUserId(), user.getMenuIds());
        return success();
    }
}
