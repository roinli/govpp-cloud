...
public class SysRoleServiceImpl implements ISysRoleService
{
    ...
    public boolean checkRoleNameUnique(SysRole role)
    {
        ...
        Long tenantId = role.getTenantId() != null ? role.getTenantId() : SecurityUtils.getTenantId();
        SysRole info = roleMapper.checkRoleNameUnique(role.getRoleName(), tenantId);
    ...
    public boolean checkRoleKeyUnique(SysRole role)
    {
        Long roleId = StringUtils.isNull(role.getRoleId()) ? -1L : role.getRoleId();
        Long tenantId = role.getTenantId() != null ? role.getTenantId() : SecurityUtils.getTenantId();
        SysRole info = roleMapper.checkRoleKeyUnique(role.getRoleKey(), tenantId);
        if (StringUtils.isNotNull(info) && info.getRoleId().longValue() != roleId.longValue())
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    {
        if (role.getTenantId() == null) role.setTenantId(SecurityUtils.getTenantId());
    ...
    public int insertRoleMenu(SysRole role)
    {
        ...
        for (Long menuId : role.getMenuIds())
        {
            ...
            rm.setMenuId(menuId);
            rm.setTenantId(role.getTenantId() != null ? role.getTenantId() : SecurityUtils.getTenantId());
            list.add(rm);
        }
        if (list.size() > 0)
        {
